...
if strcmp(pbImage.getPixelType.toString(), 'BAYER_GRBG_U8_PIXEL')
    image.rgb = permute(reshape(pixeldata,...
            [pbImage.getWidth pbImage.getHeight]),[2 1]);
    image.rgb = demosaic(image.rgb,'rggb');
elseif strcmp(pbImage.getPixelType.toString(), 'BAYER_BGGR_U8_PIXEL')
    image.rgb = permute(reshape(pixeldata,...
            [pbImage.getWidth pbImage.getHeight]),[2 1]);
    image.rgb = demosaic(image.rgb,'bggr');
else
